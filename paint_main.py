...
n_pt = 200   # number of painting times
...
print(n)
# m = STROKES_PER_BLOCK*pow((pow(2,(k+1))-1),2)
mask = mask.cpu()
mask_bg = mask_bg.cpu()
for idx in range(n):
    ...
    intermediate_canvases_bg[idx] = intermediate_canvases_bg[idx] + intermediate_canvases[n-1]*mask 
    torchvision.utils.save_image(intermediate_canvases[idx], 'results/stroke_canvases_fruit1_new/resa' + str(idx) + '.png')
    torchvision.utils.save_image(intermediate_canvases_bg[idx], 'results/stroke_canvases_fruit1_new/resa_bg' + str(idx) + '.png')


intermediate_canvases.extend(intermediate_canvases_bg)
animate_strokes_on_canvas(intermediate_canvases, image_o, "results/G_tp2_200_flower4b2_3_new.mp4", skip_every_n=1)
