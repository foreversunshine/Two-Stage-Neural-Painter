...
class NeuralCanvasStitched(nn.Module):
  ...
  def forward(self, actions: torch.Tensor, next_canvas, resize=False):
    ...
    intermediate_canvases = []
#     intermediate_canvases.append(next_canvas.detach().cpu())
    intermediate_canvases.append(F.interpolate(next_canvas.detach().cpu(), (512,512)))
...
#         for local_stroke_idx in range(random.randint(1,self.strokes_per_block)):
          ...
          if resize==False:
            ...
            intermediate_canvases.append(F.interpolate(next_canvas.detach().cpu(), (512,512)))  # Is this efficient? Maybe we should only keep it in memory in certain cases?
        block_ctr += 1
    return next_canvas, intermediate_canvases
